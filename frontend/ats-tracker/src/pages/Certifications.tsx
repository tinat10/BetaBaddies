...
export function Certifications() {
  ...
  const [sortBy, setSortBy] = useState<'earned' | 'expiration' | 'name'>('expiration')
  ...
  useEffect(() => {
    ...
    // Apply sorting
    if (sortBy === 'earned') {
      filtered.sort((a, b) => new Date(b.date_earned).getTime() - new Date(a.date_earned).getTime())
  ...
  const getStatistics = () => {
    const stats = { total: 0, active: 0, expiring: 0, expired: 0, permanent: 0 }
  ...
  const handleUpdateCertification = async () => {
    ...
    try {
      ...
      alert(err.message || 'Failed to update certification')
    }
  }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'active': return { color: 'bg-green-100 text-green-700 border-green-200', icon: 'mingcute:check-circle-fill', label: 'Active' }
      case 'expiring': return { color: 'bg-amber-100 text-amber-700 border-amber-200', icon: 'mingcute:alert-fill', label: 'Expiring Soon' }
      case 'expired': return { color: 'bg-red-100 text-red-700 border-red-200', icon: 'mingcute:close-circle-fill', label: 'Expired' }
      case 'permanent': return { color: 'bg-indigo-100 text-indigo-700 border-indigo-200', icon: 'mingcute:infinity-fill', label: 'Permanent' }
      default: return { color: 'bg-gray-100 text-gray-700 border-gray-200', icon: 'mingcute:question-fill', label: 'Unknown' }
    }
  }
    ...
    <div className="p-10 max-w-[1400px] mx-auto bg-white font-sans min-h-full">
      ...
      {stats.expiring > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 flex items-center gap-3">
          <Icon icon="mingcute:alert-fill" width={24} className="text-amber-600" />
          <p className="text-sm text-amber-800">
            ⚠️ You have <strong>{stats.expiring}</strong> certification{stats.expiring > 1 ? 's' : ''} expiring within 30 days
          </p>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl p-6 mb-6 shadow-sm border border-slate-100">
        <div className="flex flex-wrap items-center gap-6">
          <div className="flex items-center gap-2">
            <Icon icon="mingcute:certificate-line" width={20} className="text-slate-600" />
            <span className="text-slate-600 font-medium">{stats.total} Total</span>
          </div>
          <div className="flex items-center gap-2">
            <Icon icon="mingcute:check-circle-fill" width={20} className="text-green-500" />
            <span className="text-slate-600">{stats.active} Active</span>
          </div>
          <div className="flex items-center gap-2">
            <Icon icon="mingcute:alert-fill" width={20} className="text-amber-500" />
            <span className="text-slate-600">{stats.expiring} Expiring</span>
          </div>
          <div className="flex items-center gap-2">
            <Icon icon="mingcute:close-circle-fill" width={20} className="text-red-500" />
            <span className="text-slate-600">{stats.expired} Expired</span>
          </div>
          <div className="flex items-center gap-2">
            <Icon icon="mingcute:infinity-fill" width={20} className="text-indigo-500" />
      ...
      <div className="bg-white rounded-2xl p-6 mb-6 shadow-sm border border-slate-100">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="flex gap-2">
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as 'earned' | 'expiration' | 'name')}
        ...
        <div className="mt-4 flex items-center gap-3 text-sm">
          <span className="text-slate-600 font-medium">{filteredCertifications.length} certifications</span>
          {searchTerm && (
      ...
      {filteredCertifications.length === 0 ? (
        <div className="text-center py-20 bg-slate-50 rounded-2xl border border-slate-100">
          <Icon icon="mingcute:certificate-line" width={64} className="mx-auto text-slate-300 mb-4" />
          <h3 className="text-xl font-semibold text-slate-900 mb-2">No certifications found</h3>
          <p className="text-slate-600 mb-6">Start by adding your first certification</p>
          <button
            onClick={openAddModal}
          ...
          {filteredCertifications.map((cert) => {
            ...
            return (
              ...
              >
                ...
                {/* Certification Name */}
                <h3 className="text-xl font-semibold text-slate-900 mb-2">{cert.name}</h3>
                ...
                {cert.daysUntilExpiration !== null && !cert.never_expires && (
                  <div className={`text-sm font-medium mb-4 ${
                    (cert.daysUntilExpiration || 0) < 0 ? 'text-red-600' : 
                    (cert.daysUntilExpiration || 0) <= 30 ? 'text-amber-600' : 
                    'text-green-600'
                  }`}>
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <h3 className="text-xl font-semibold text-slate-900">{cert.name}</h3>
                    </div>
                        ...
                        <>
                          ...
                          {cert.daysUntilExpiration !== null && (
                            <span className={`font-medium ${
                              ...
                              cert.daysUntilExpiration <= 30 ? 'text-amber-600' : 
                              'text-green-600'
                            }`}>
                              {formatDaysUntilExpiration(cert.daysUntilExpiration)}
                            </span>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, never_expires: e.target.checked, expiration_date: '' })}
                  className="w-4 h-4 text-blue-500 border-slate-300 rounded focus:ring-blue-500"
                />
                <label htmlFor="neverExpires" className="text-sm font-medium text-slate-700">
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                onClick={showAddModal ? handleAddCertification : handleUpdateCertification}
              ...
              return (
                <>
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <h2 className="text-3xl font-bold text-slate-900 mb-2">{selectedCertification.name}</h2>
                      <span className={`inline-flex items-center gap-1 px-4 py-2 rounded-full text-sm font-medium border ${badge.color}`}>
                  ...
                  <div className="space-y-6">
                    {/* Organization */}
                    <div>
                      <h3 className="text-lg font-semibold text-slate-900 mb-2">Issuing Organization</h3>
                      <div className="flex items-center gap-2 text-slate-600">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <h4 className="text-sm font-medium text-slate-500 mb-1">Date Earned</h4>
                        <p className="text-slate-900">{formatDate(selectedCertification.date_earned)}</p>
                      </div>
                      <div>
                        <h4 className="text-sm font-medium text-slate-500 mb-1">Expiration Date</h4>
                        {selectedCertification.never_expires ? (
                          ...
                          <p className="text-slate-900">{formatDate(selectedCertification.expiration_date || '')}</p>
                        )}
                      </div>
                    </div>

                    {/* Days Until Expiration */}
                    {selectedCertification.daysUntilExpiration !== null && !selectedCertification.never_expires && (
                      <div>
                        <h4 className="text-sm font-medium text-slate-500 mb-1">Status</h4>
                        <p className={`font-medium ${
                          (selectedCertification.daysUntilExpiration || 0) < 0 ? 'text-red-600' : 
                          (selectedCertification.daysUntilExpiration || 0) <= 30 ? 'text-amber-600' : 
                          'text-green-600'
                        }`}>
                          {formatDaysUntilExpiration(selectedCertification.daysUntilExpiration)}
                        </p>
                      </div>
                    )}
                  </div>
