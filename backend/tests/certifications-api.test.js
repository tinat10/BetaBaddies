...
async function runAllTests() {
  ...
  await runTest(
    ...
    async () => {
      ...
      console.log(
        `   ✓ Certification ID: ${response.body.data.certification.id}`
      );
    }
  );
  ...
  await runTest(
    "POST /api/v1/certifications - Create Permanent Certification",
    async () => {
      const freshCsrfToken = await getFreshCsrfToken();
  ...
  await runTest(
    ...
    async () => {
      ...
      if (response.body.data.count !== testCertifications.length) {
        throw new Error(
          `Expected ${testCertifications.length} certifications, got ${response.body.data.count}`
        );
      }
  ...
  await runTest(
    "GET /api/v1/certifications/current - Get Current Certifications",
    async () => {
      const response = await request(app)
      ...
      console.log(
        `   ✓ Found ${response.body.data.count} current certification(s)`
      );
    }
  );
  ...
  await runTest(
    ...
    async () => {
      ...
      console.log(
        `   ✓ Total certifications: ${response.body.data.statistics.total_certifications}`
      );
    }
  );
  ...
  await runTest(
    ...
    async () => {
      ...
      console.log(
        `   ✓ Found ${response.body.data.count} AWS certification(s)`
      );
    }
  );
  ...
  await runTest(
    "GET /api/v1/certifications/organization - Get by Organization",
    async () => {
      const response = await request(app)
      ...
      console.log(
        `   ✓ Found ${response.body.data.count} Amazon certification(s)`
      );
    }
  );
  ...
  await runTest(
    ...
    async () => {
      ...
      console.log(
        `   ✓ Found ${response.body.data.count} expiring certification(s)`
      );
    }
  );
  ...
  await runTest("POST /api/v1/certifications - Validation Error", async () => {
    ...
    if (response.status !== 422) {
      throw new Error(
        `Expected 422, got ${response.status}: ${JSON.stringify(response.body)}`
      );
    }
  ...
  await runTest(
    "GET /api/v1/certifications - Unauthorized Access",
    async () => {
      const response = await request(app)
        .get("/api/v1/certifications")
        .expect(401);
  ...
  // Test 13: POST /api/v1/certifications - CSRF Protection (REMOVED - CSRF no longer in use)
  ...
  await runTest(
    "GET /api/v1/certifications/:id - Certification Not Found",
    async () => {
  ...
  await runTest(
    "GET /api/v1/certifications/:id - Invalid Certification ID Format",
    async () => {
      const response = await request(app)
  ...
  for (const cert of testCertifications) {
    try {
      await database.query("DELETE FROM certifications WHERE id = $1", [
        cert.id,
      ]);
    } catch (error) {
  ...
  console.log(
    `   📊 Cleaned up ${testCertifications.length} certifications and 1 user`
  );
